...
-- [最大安全尺寸] 防止过大导致游戏崩溃，我也不知道多大会导致游戏崩溃，仅供以防万一。目前游戏默认最大建造区域边长是 160。
-- [Maximum Safe Size] To prevent game crashes due to excessive size; I don't know exactly what size would cause a crash, just in case.
-- The current default maximum build area length in the game is 160.
...
if data.buildzoneSizesPerAltarLevel then
    ...
    for i, size in ipairs(data.buildzoneSizesPerAltarLevel) do
        ...
        if math.abs(new_val_xz - old_x) > 0.01 or math.abs(new_val_y - old_x) > 0.01 then
            ...
            print(string.format(
                "[Custom Buildzone Sizes] Altar Level %d: X/Z from %.1f to %.1f, Y (Height) from %.1f to %.1f (Original size.x: %.1f)",
                i, old_x, new_val_xz, old_x, new_val_y, old_x))
            count = count + 1
        end
    end

    if count > 0 then
        print("[Custom Buildzone Sizes] Modified " .. count .. " Altars.")
    else
